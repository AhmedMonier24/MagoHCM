...
p6 = new Product ('emt','Event Management Tool Solution',
    ...
    './assets/images/productModel/event.jpg');
...
var mobileMenuNavigation = document.querySelectorAll(".mobile_menu_links a");
...
var vacancies = ['FrontEnd Developer','Business Analysist','Fullstack Developer','Quality Control']

 
//display client logos
var imgFlow = document.createElement('div');
// var mobileImages_width = document.querySelector('#clients-mobile').clientWidth;
...
imageArray.forEach((image,index) => {
    ...
    // imgMobile.setAttribute('src', imageArray[index]);
    // imgMobile.width = mobileImages_width/3;

    // var mobileContainer = document.createElement('div');
    // mobileContainer.append(imgMobile);
    // console.log(mobileImages_width/3);
...
var templateOfImages = ``;
var mobileImagesArray_Clients = document.querySelector('#clients-mobile');

for(let i = 1; i < 13; i++)
{
    templateOfImages += `<img src="./assets/images/cients/${i}.jpg"/>`
}

mobileImagesArray_Clients.innerHTML = templateOfImages;

//job vacancies display
vacancies.forEach((job, i)=>{
    var jobtitle = document.createElement('div');
    // jobtitle.setAttribute('data-aos',"fade-left");
    // jobtitle.setAttribute('data-aos-duration',`${i+2}+000`);
...
const linkHighlight = (navElement) =>{
    navElements.forEach(element=> 
        ...
        mobileMenuNavigation.forEach(element=> 
            element.classList.remove("active"))
...
mobileMenuNavigation.forEach(e => 
    e.addEventListener('click',e => {
        linkHighlight(e.target) 
        closeMenuFunction();
    }))
...
var mobile_only_button = document.querySelector(".mobile_only");

mobile_only_button.addEventListener('click' , () => {
    let mobile_menu = document.querySelector('.mobile_menu');
...
var close_mobile_menu = document.querySelector('#close_mobile_menu');
...
const goToLink = (link) => {
    window.location.href = `https://www.google.com/maps/place${link.trim()}`;
}